...
\begin{figure}[h]
    \centering
    % \includegraphics[width=0.8\textwidth]{isr_flowchart.png}
...
For the serious game (developed in Unity or Godot), a custom communication library was developed to interface with the MONEEE hardware.
...
XDF is the native format for the Lab Streaming Layer (LSL). It supports multi-modal data containers, allowing the EEG samples and the discrete event markers to be stored in parallel streams with unified timestamps, ready for post-processing in MATLAB (EEGLAB) or Python (MNE).