...
\subsection{Electrode Interfaces and EEG Cap Deployment}

The fundamental first step in acquiring high-fidelity electroencephalographic signals is securing a stable, low-impedance physical connection to the scalp. To accommodate pediatric populations and highly active users where traditional abrasive skin preparation is challenging, some modern designs have transitioned toward active dry sensors or polymer-based microneedle arrays \cite{liu2024multichannel, kim2024skin}. While embedding unity-gain operational amplifiers at the scalp site can buffer the inherently high skin-electrode impedance of dry contacts—often exceeding 100 k$\Omega$ \cite{xiong2025advancements}—and reduce cable noise \cite{giangrande2024motion}, dry technologies still struggle to match the pristine signal-to-noise ratio and contact stability of wet silver/silver-chloride (Ag/AgCl) electrodes.

The physical deployment of the EEG cap itself remains a critical bottleneck in the real-world acquisition pipeline. A review of the commercial landscape distinguishes systems not only by electrode type but by their mounting methodology \cite{petrossian2023advances}. Systems utilizing wet electrodes, like the Brain Products actiCAP \cite{BrainProducts_ActiCap}, deliver exceptional stability and sub-5~k$\Omega$ impedances, but setting up a clinical montage demands significant time (often 30--60 minutes) due to the necessity of precisely parting the hair and injecting conductive gel at every site \cite{niso2023wireless}. This extended preparation is particularly problematic for pediatric patients or children with ADHD, whose restlessness and limited compliance window directly conflict with prolonged setup procedures \cite{Lim2023}.

In contrast, consumer and research-grade dry systems—such as the Emotiv EPOC X \cite{Emotiv_EPOCX}, Bitbrain Diadem \cite{Bitbrain_Diadem}, and g.tec g.Nautilus \cite{Gtec_GNautilusProFlexible}—prioritize rapid deployment, often achieving full setup in under five minutes \cite{he2023diversity}. However, these rely on rigid or semi-rigid headpieces that may not optimally conform to varying cranial anatomies, creating localized pressure points and variable contact quality. Open-source frameworks like OpenBCI \cite{OpenBCI_CytonDaisy, ariza2022low} provide highly capable electronics but generally rely on generic, fabric-based caps lacking specialized mechanisms for rapid seating. The fundamental challenge is bridging the gap between the deployment speed of dry electrodes and the electrical reliability of wet electrodes. This motivates the design of custom cap architectures that retain gold-standard wet Ag/AgCl electrodes but streamline the application process. By optimizing the anatomical fit and integrating real-time software-level impedance checks, it is possible to minimize patient discomfort, drastically reduce preparation time, and quickly achieve stable, low-impedance scalp connections.

\begin{table}[htbp]
    \renewcommand{\arraystretch}{1.2}
    \caption{Electrode interfaces and mounting methodologies across commercial BCI platforms.}
    \label{table:bci_electrodes}
    \centering
    \footnotesize
    \begin{tabularx}{\textwidth}{@{}
        ...
        c @{}}
        \toprule
        \textbf{Device}                              & \textbf{Company}    & \textbf{Electrodes}  \\ \midrule
        Cyton + Daisy \cite{OpenBCI_CytonDaisy}      & OpenBCI             & Flexible / Wet / Dry \\
        actiCAP \cite{BrainProducts_ActiCap}         & Brain Products GmbH & Flexible / Wet / Dry \\
        EPOC X \cite{Emotiv_EPOCX}                   & Emotiv              & Rigid / Wet          \\
        Diadem \cite{Bitbrain_Diadem}                & Bitbrain            & Rigid / Dry          \\
        g.Nautilus \cite{Gtec_GNautilusProFlexible}  & g.tec               & Flexible             \\
        Ambulatory platform \cite{pinho2014wireless} & --                  & Active / Dry         \\
        Neurofeedback system \cite{Totev2023}        & --                  & Passive / Dry        \\
        BEATS \cite{Beats}                           & --                  & Flexible / Wet       \\ \bottomrule
    \end{tabularx}
\end{table}
...
The practical realization of this heterogeneous computing philosophy relies profoundly on the selection of appropriate embedded hardware platforms that natively support such strict partitioning. For the deterministic acquisition gateway (the low-power microcontroller), the ARM Cortex-M architecture has established itself as the ubiquitous industry standard \cite{Beats}. Specifically, cores like the ARM Cortex-M4F found in the Texas Instruments TM4C1294 series \cite{ti_tm4c1294} are engineered explicitly for time-critical, mixed-signal applications. They feature a single-cycle hardware Floating-Point Unit (FPU) for immediate, on-the-fly signal scaling, and a Highly Deterministic Nested Vectored Interrupt Controller (NVIC). This deterministic interrupt handling ensures that when the analog front-end asserts a Data Ready (DRDY) signal, the ARM core services the interrupt in a highly predictable, microsecond-scale timeframe \cite{battaglia2022eeg}, ensuring that every single sample is captured and timestamped with zero software jitter.

Conversely, the secondary computing node—tasked with asynchronous data aggregation, network routing, and massive data storage—requires the robust capabilities of a full Microprocessor Unit (MPU) operating a Linux-based OS. The Raspberry Pi Compute Module 4 (CM4) \cite{raspberrypi_cm4} has emerged as an exceptionally powerful and adaptable platform for this tier. By housing a quad-core ARM Cortex-A72 processor, GBs of high-speed RAM, and extensive connectivity matrices within a compact, carrier-board-deployable form factor, the CM4 possesses the precise computational bandwidth needed to run complex middleware (like the Lab Streaming Layer) and handle bulk storage simultaneously \cite{Totev2023}. Crucially, pairing the real-time, bare-metal reliability of the TM4C1294 with the high-level computational versatility of the Raspberry Pi CM4 creates a robust, two-tiered architecture \cite{pinho2014wireless}. The ARM chip acts as an impenetrable firewall that protects the temporal integrity of the incoming bio-signal, allowing the Raspberry Pi to execute heavy data management and network synchronization tasks without ever impacting the flawless timing of the baseline biological data stream \cite{xie2022fpga}.
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{%
        \begin{forest}
...
In summary, while current literature provides comprehensive methodologies for EEG acquisition, adapting these for clinical-grade pediatric \gls{ADHD} assessment requires navigating specific physical and computational limitations. Addressing the first challenge---rapid and reliable physical deployment---custom cap designs utilizing wet electrodes combined with real-time software impedance feedback bridge the critical gap by streamlining the application process while maintaining excellent signal stability. For the second challenge---overcoming \gls{SNR} limitations and resource exhaustion---dedicated high-resolution analog front-ends combined with active grounding and distributed edge-offloading present the most viable architecture. However, many current statistical and deep learning-based denoising alternatives impose computational overheads that inherently threaten real-time stability on constrained systems. Consequently, outcomes for this computational problem must be explicitly measured by quantifying baseline input-referred noise against commercial standards and by conducting stress tests that monitor continuous CPU and RAM utilization to preclude data packet loss during high-frequency streaming. For the third challenge---temporal synchronization and latency variability---relying solely on software-level middleware or network protocols is limited by unpredictable operating system scheduling and display refresh delays. The most promising alternative rests on hardware-bounded event marking synchronized via explicit interrupt service routines. To validate outcomes for this synchronization challenge, evaluations must rigorously quantify short-term, non-deterministic communication jitter and measure cumulative clock drift over extended, full-length therapeutic sessions. Finally, these temporal metrics must be biologically anchored by executing physiological baseline validations, such as the alpha-band attenuation test, alongside strict signal isolation assessments to verify that the extracted, time-locked biomarkers are free from spatial crosstalk and genuinely reflect neurocognitive engagement.
